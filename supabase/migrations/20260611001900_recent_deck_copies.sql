create index if not exists deck_copy_events_deck_occurred_idx
  on public.deck_copy_events (deck_id, occurred_on desc);

drop function public.search_public_decks(
  text, text, text, text, text, boolean, text, integer, integer
);

create function public.search_public_decks(
...
returns table (
  ...
  copy_count bigint,
  recent_copy_count bigint,
  trend_score smallint,
  tracking_source_count integer,
...
as $$
  with catalog as (
    select
      ...
      d.comment_count,
      d.copy_count,
      coalesce(recent_copies.copy_count, 0)::bigint as recent_copy_count,
      d.trend_score,
      d.tracking_source_count,
      d.last_tracked_at,
      d.published_at,
      d.updated_at
    from public.decks d
    ...
    left join lateral (
      ...
      from public.deck_copy_events event
      where event.deck_id = d.id
        and event.occurred_on >= current_date - 6
    ) recent_copies on true
  ...
  filtered as (
    ...
    where (
        ...
        or author_name ilike '%' || btrim(p_query) || '%'
      )
  ...
  select
    ...
    filtered.copy_count,
    filtered.recent_copy_count,
    filtered.trend_score,
    filtered.tracking_source_count,
    filtered.last_tracked_at,
    filtered.published_at,
    filtered.updated_at
  from filtered
  order by
    case when p_sort = 'trending' then recent_copy_count end desc,
    case when p_sort = 'trending' then trend_score end desc,
    case when p_sort = 'trending' then last_tracked_at end desc nulls last,
